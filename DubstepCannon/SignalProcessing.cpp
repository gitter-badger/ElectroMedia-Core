...
// ProcessWindow(int*, double*, int)
// ---
// Confused about this. Needs to be redefined and renamed to something more specific.
double* prepareAndExecuteFFT(const int* dataIn)
{
	...
	for(int i=0; i<convertFrequencyToInt(MAXIMUM_FREQUENCY_ACCOUNTED); i++) // What is the 230?
	{		
		// Convert to frequency domain from complex results
		dataOut[i] = sqrt(results[i][0]*results[i][0] + results[i][1]*results[i][1]);

		myfile << (BOUNDARY_CONVERSION_SCALAR*i - BOUNDARY_CONVERSION_OFFSET) << "," << int(dataOut[i]) << endl;
	}
